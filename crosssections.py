...
def get_matches(inputkey):
    matches = []
    for k in xsdict.keys():
        if k.lower().startswith(inputkey.lower()) or inputkey.lower().startswith(k.lower()):
            matches.append(k)
    matches.sort(key=len, reverse=True)
    return matches

def get_xs(inputkey, verbose=True, year=18, multiply_by_br=True, only_best_match=True):
    xss = []
    for key in get_matches(inputkey):
        xs = None
        try:
            xs = xsdict[key]['CrossSection']['XS_13TeV']
        except KeyError:
            try:
                xs = xsdict[key]['CrossSection']['XS_20' + str(year)]
            except KeyError:
                print(
                    'Key {0} matched but could not retrieve a cross section'
                    .format(key)
                    )

        if not(xs is None):
            if verbose:
                print('{0}: Match \'{1}\' yields xs {2}'.format(inputkey, key, xs))
            if multiply_by_br:
                try:
                    br = xsdict[key]['BranchingRatio']['BR_13TeV']
                    print('{0}: Found BR: {1} x {2} = {3} <-- eff xs'.format(inputkey, xs, br, xs*br))
                    xs *= br
                except KeyError:
                    pass

            if only_best_match: return xs
            xss.append(xs)
    else:
        if only_best_match:
            return None
