...
class Dataset(object):
    def __init__(self, name, rootfiles, treename='TreeMaker2/PreSelection', is_signal=False):
        super().__init__()
        self.name = name
...
def init_ttjets_datasets():
    return [ Dataset(name, get_bkg_rootfiles(name)) for name in ttjets ]

def init_qcd_datasets():
    return [ Dataset(name, get_bkg_rootfiles(name)) for name in qcd ]

def init_wjets_datasets():
    return [ Dataset(name, get_bkg_rootfiles(name)) for name in wjets ]

def init_zjets_datasets():
    return [ Dataset(name, get_bkg_rootfiles(name)) for name in zjets ]